...
public class ItemSmiley {
    ...
    public void draw(SpriteBatch batch, float x, float y, boolean gold, int rotation) {
        // TextureRegion region, float x, float y, float originX, float originY, float width, float height,
        //		float scaleX, float scaleY, float rotation
        x -= 5;
        y -= 5;
        if (!gold) batch.draw(texture, x, y, 13, 13, 26, 26, 1, 1, rotation);
        else batch.draw(textureGold, x, y, 13, 13, 26, 26, 1, 1, rotation);
    }
}

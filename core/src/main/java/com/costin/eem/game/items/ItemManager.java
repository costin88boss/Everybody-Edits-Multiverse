...
public class ItemManager {
    ...
    private final ArrayList<ItemSmiley> smilies = new ArrayList<>();
    private final ArrayList<ItemAura> auraShapes = new ArrayList<>();
    private final ArrayList<ItemNpc> npcs = new ArrayList<>();
    private final ArrayList<ItemBrickPackage> brickPackages = new ArrayList<>();
    //Quick lookup for drawing system!
    private final TextureRegion[] brickTextures = new TextureRegion[4001];
    private final ItemBrick[] bricks = new ItemBrick[4001];
    ...
    public static void initialize(boolean initGraphics) {
        if (singleton != null) return; //throw new RuntimeException("Cannot initialize ItemManager more than once.");
        singleton = new ItemManager();

        if (initGraphics) singleton.loadTextures();
    ...
    public static ItemManager instance() {
        if (singleton == null) {
            throw new RuntimeException("ItemManager not initialized. use initialize(boolean) to initialize.");
        }
        return singleton;
    }
    ...
    private void initializeManager() {
        ...
        basicbg.addBrick(createBrick(500, ItemLayer.BACKGROUND, bgBlocksBMD, "", "", ItemTab.BACKGROUND, false, false, 0 /* 500 */, -1, new String[]{"Gray", "Grey"}));
...
//			water.addBrick(createBrick(ItemId.WATER, ItemLayer.ABOVE, waterBMD, "", ItemTab.ACTION,false,false,0,0x0))
    ...
    private void loadTextures() {
        ...
        aurasOrnateBMD = new Texture("media/auras_ornate.png");
